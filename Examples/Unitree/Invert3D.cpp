...
int main() {
    ...
    for (size_t i = 0; i < joint_index_map.size(); ++i) {
        std::cout << "Lower joint index: " << i + 1 << ", Upper joint index: " << joint_index_map[i] << std::endl;
    }
    ...
    for (size_t i = 0; i < jointTrajectoriesLower.size(); ++i) {
        ...
        for (pinocchio::JointIndex j = 1; j < model_upper.njoints; ++j) {
            ...
            std::cout << "Joint " << model_upper.names[j] << " position: " << joint_pos.transpose() << std::endl;
        }
        ...
        for (pinocchio::JointIndex j = 1; j < model_upper.njoints; ++j) {
            const auto& joint_pos = data_upper.oMi[j].translation();
            std::cout << joint_pos[0] << " " << joint_pos[1] << " " << joint_pos[2] << " ";
