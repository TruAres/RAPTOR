#include "H1MultipleStepPeriodicityConstraints.h"

namespace RAPTOR {
namespace H1 {

H1MultipleStepPeriodicityConstraints::H1MultipleStepPeriodicityConstraints(std::shared_ptr<Trajectories>& currTrajPtr_input,
                                                                           std::shared_ptr<Trajectories>& nextTrajPtr_input,
                                                                           std::shared_ptr<H1ConstrainedInverseDynamics> currDcidPtr_input,
                                                                           std::shared_ptr<H1ConstrainedInverseDynamics> nextDcidPtr_input,
    ...
    if (currTrajPtr_->varLength != nextTrajPtr_->varLength) {
        throw std::invalid_argument("H1MultipleStepPeriodicityConstraints: currTrajPtr_ and nextTrajPtr_ should have the same varLength");
    }
...
void H1MultipleStepPeriodicityConstraints::compute(const VecX& z, 
                                                   bool compute_derivatives,
                                                   bool compute_hessian) {
    if (compute_hessian) {
        throw std::invalid_argument("H1MultipleStepPeriodicityConstraints does not support hessian computation");
    }

    Model& model = *(currDcidPtr_->modelPtr_);
    Data& data = *(currDcidPtr_->dataPtr_);
    std::shared_ptr<H1DynamicsConstraints> nextDdcPtr_ = nextDcidPtr_->ddcPtr_;
...
void H1MultipleStepPeriodicityConstraints::compute_bounds() {
...
void H1MultipleStepPeriodicityConstraints::print_violation_info() {
    // (1) H * (v+ - v-) = J * lambda
    for (int i = 0; i < NUM_JOINTS; i++) {
        if (abs(g1(i)) >= 1e-4) {
            std::cout << "        H1MultipleStepPeriodicityConstraints.cpp: H * (v+ - v-) = J * lambda: dim " 
                      << i 
                      << " is violated: "
                      << g(i) 
                      << std::endl;
    ...
    for (int i = 0; i < NUM_DEPENDENT_JOINTS; i++) {
        if (abs(g2(i)) >= 1e-4) {
            std::cout << "        H1MultipleStepPeriodicityConstraints.cpp: J * v+ = 0: dim " 
                      << i
                      << " is violated: "
                      << g2(i)
                      << std::endl;
    ...
    for (int i = 0; i < NUM_INDEPENDENT_JOINTS; i++) {
        if (abs(g3(i)) >= 1e-4) {
            std::cout << "        H1MultipleStepPeriodicityConstraints.cpp: position reset: dim " 
                      << i
                      << " is violated: "
                      << g3(i)
                      << std::endl;
    ...
    for (int i = 0; i < NUM_INDEPENDENT_JOINTS; i++) {
        if (abs(g4(i)) >= 1e-4) {
            std::cout << "        H1MultipleStepPeriodicityConstraints.cpp: velocity reset: dim " 
                      << i
                      << " is violated: "
                      << g4(i)
                      << std::endl;
    ...
    // (5) contact constraints
    if (g5(0) <= -1e-4) {
        std::cout << "        H1MultipleStepPeriodicityConstraints.cpp: positive contact force is violated: " 
                  << g5(0)
                  << std::endl;
    }
    if (g5(1) >= 1e-4) {
        std::cout << "        H1MultipleStepPeriodicityConstraints.cpp: translation friction cone is violated: " 
                  << g5(1)
                  << std::endl;
    }
    if (g5(2) >= 1e-4) {
        std::cout << "        H1MultipleStepPeriodicityConstraints.cpp: rotation friction cone is violated: " 
                  << g5(2)
                  << std::endl;
    }
    if (g5(3) >= 1e-4) {
        std::cout << "        H1MultipleStepPeriodicityConstraints.cpp: ZMP on one axis is violated: " 
                  << g5(3)
                  << std::endl;
    }
    if (g5(4) >= 1e-4) {
    ...
    if (g5(5) >= 1e-4) {
        std::cout << "        H1MultipleStepPeriodicityConstraints.cpp: ZMP on the other axis is violated: " 
                  << g5(5)
                  << std::endl;
    }
    if (g5(6) >= 1e-4) {
