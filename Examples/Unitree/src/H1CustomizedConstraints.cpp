#include "H1CustomizedConstraints.h"

namespace RAPTOR {
namespace H1 {

H1CustomizedConstraints::H1CustomizedConstraints(const Model& model_input,
                                                 std::shared_ptr<Trajectories>& trajPtr_input,
                                                 std::shared_ptr<H1DynamicsConstraints>& ddcPtr_input,
...
void H1CustomizedConstraints::compute(const VecX& z, 
...
void H1CustomizedConstraints::compute_bounds() {
    // (1) swingfoot height always larger than 0
    //               height equal to 0 at the beginning and at the end
    //               height higher than the desired value in the middle
...
void H1CustomizedConstraints::print_violation_info() {
    // (1) swingfoot height always larger than 0
    //               height equal to 0 at the beginning and at the end
    //               height higher than the desired value in the middle
    for (int i = 0; i < trajPtr_->N; i++) {
        if (g1(i) <= g1_lb(i) - 1e-4) {
            std::cout << "        H1CustomizedConstraints.cpp: swing foot height at time instance " 
                      << i 
    ...
    for (int i = 0; i < trajPtr_->N; i++) {
        if (abs(g2(i)) >= 1e-4) {
            std::cout << "        H1CustomizedConstraints.cpp: swing foot roll at time instance " 
                      << i 
                      << " is not close to 0: " 
                      << g2(i) 
                      << std::endl;
        ...
        if (abs(g3(i)) >= 1e-4) {
            std::cout << "        H1CustomizedConstraints.cpp: swing foot pitch at time instance " 
                      << i 
                      << " is not close to 0: " 
                      << g3(i) 
                      << std::endl;
        ...
        if (abs(g4(i)) >= 1e-4) {
            std::cout << "        H1CustomizedConstraints.cpp: swing foot yaw at time instance " 
                      << i 
                      << " is not close to 0: " 
                      << g4(i) 
                      << std::endl;
    ...
    if (g5(0) <= g5_lb(0) - 1e-4 ||
        g5(0) >= g5_ub(0) + 1e-4) {
        std::cout << "        H1CustomizedConstraints.cpp: swing foot x at beginning is not equal to desired value: " 
                  << g5(0) 
                  << std::endl;
    }
    if (g5(1) <= g5_lb(1) - 1e-4 ||
        g5(1) >= g5_ub(1) + 1e-4) {
        std::cout << "        H1CustomizedConstraints.cpp: swing foot x at end is not equal to desired value: " 
                  << g5(1) 
                  << std::endl;
    }
